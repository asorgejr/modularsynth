...
class MidiInputNode : public Module {
  
private:
  
  const std::string kMidiInputName = "MidiInput";
  
public:
  
  MidiInputNode() : Module() { this->setName("Midi In"); }

  ~MidiInputNode() { removeAllChildren(); }
  ...
  NodeDefinition getNodeDefinition() override {
    const NodeDefinition def = NodeDefinition(kMidiInputName,
      FlowDataList {},
      FlowDataList {FlowDataType::CV},
      [=](ModularGraphView *host, const Point<float> &pos) -> NodeComponent* {
        return host->addNode(std::make_unique<MidiInputNode>(), pos);
      });
    return def;
  }
