...
MProcessor::ModularAudioProcessor(sptr<AudioDeviceManager> audioDeviceManager)
...
void MProcessor::initialiseGraph() {
  ...
  masterAudioIn  = mainProcessor->addNode(
    static_cast<unique_ptr<AudioProcessor>>(
...
MGraphView::ModularNodeGraphView(sptr<AudioProcessorGraph> audioGraph) :
GraphViewComponent(), audioGraph(move(audioGraph)) {}
